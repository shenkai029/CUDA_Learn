	...
	int* temp, int size)
{
	...
	// the reduction sum result stores at first element (tid == 0) of the block
	// so assign the result it to temp array using block idx
	if (tid == 0)
	{
	...
	int* int_array, int* temp_array, int size)
{
	...
	for (int offset = 1; offset <= blockDim.x / 2; offset *= 2)
	{
		// use consecutive neigbour thread to process sum of offset elements 
	...
	int* temp_array, int size)
{
	...
	{
		// reduce use of threads based on each iteration offset
		if (tid < offset)
		{
